	...
	abstract class QResizableGen extends QControl	{
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				case 'Animate':
					try {
						$this->blnAnimate = \QCubed\Type::Cast($mixValue, \QCubed\Type::BOOLEAN);
				...
				case 'AnimateEasing':
					try {
						$this->strAnimateEasing = \QCubed\Type::Cast($mixValue, \QCubed\Type::STRING);
				...
				case 'AutoHide':
					try {
						$this->blnAutoHide = \QCubed\Type::Cast($mixValue, \QCubed\Type::BOOLEAN);
				...
				case 'Ghost':
					try {
						$this->blnGhost = \QCubed\Type::Cast($mixValue, \QCubed\Type::BOOLEAN);
				...
				case 'Helper':
					try {
						$this->strHelper = \QCubed\Type::Cast($mixValue, \QCubed\Type::STRING);
		...
		**/
		public static function GetModelConnectorParams() {
			return array_merge(parent::GetModelConnectorParams(), array(
				new QModelConnectorParam (get_called_class(), 'Animate', 'Animates to the final size after resizing.', \QCubed\Type::BOOLEAN),
				new QModelConnectorParam (get_called_class(), 'AnimateEasing', 'Which easing to apply when using the animate option.', \QCubed\Type::STRING),
				new QModelConnectorParam (get_called_class(), 'AutoHide', 'Whether the handles should hide when the user is not hovering over theelement.', \QCubed\Type::BOOLEAN),
				new QModelConnectorParam (get_called_class(), 'Delay', 'Tolerance, in milliseconds, for when resizing should start. Ifspecified, resizing will not start until after mouse is moved beyondduration. This can help prevent unintended resizing when clicking onan element.', \QCubed\Type::INTEGER),
				new QModelConnectorParam (get_called_class(), 'Disabled', 'Disables the resizable if set to true.', \QCubed\Type::BOOLEAN),
				new QModelConnectorParam (get_called_class(), 'Distance', 'Tolerance, in pixels, for when resizing should start. If specified,resizing will not start until after mouse is moved beyond distance.This can help prevent unintended resizing when clicking on an element.', \QCubed\Type::INTEGER),
				new QModelConnectorParam (get_called_class(), 'Ghost', 'If set to true, a semi-transparent helper element is shown forresizing.', \QCubed\Type::BOOLEAN),
				new QModelConnectorParam (get_called_class(), 'Grid', 'Snaps the resizing element to a grid, every x and y pixels. Arrayvalues: [ x, y ].', \QCubed\Type::ARRAY_TYPE),
				new QModelConnectorParam (get_called_class(), 'Helper', 'A class name that will be added to a proxy element to outline theresize during the drag of the resize handle. Once the resize iscomplete, the original element is sized.', \QCubed\Type::STRING),
				new QModelConnectorParam (get_called_class(), 'MaxHeight', 'The maximum height the resizable should be allowed to resize to.', \QCubed\Type::INTEGER),
				new QModelConnectorParam (get_called_class(), 'MaxWidth', 'The maximum width the resizable should be allowed to resize to.', \QCubed\Type::INTEGER),
				new QModelConnectorParam (get_called_class(), 'MinHeight', 'The minimum height the resizable should be allowed to resize to.', \QCubed\Type::INTEGER),
				new QModelConnectorParam (get_called_class(), 'MinWidth', 'The minimum width the resizable should be allowed to resize to.', \QCubed\Type::INTEGER),
			));
		}
	}