 ...
 * Class Node
 *
 * A table column that displays the content of a database column represented by a QQNode\AbstractBase object.
...
class Node extends Property
{
    ...
    {
        if ($objNodes instanceof QQNode\AbstractBase) {
            $objNodes = [$objNodes];
        } elseif (empty($objNodes) || !is_array($objNodes) || !$objNodes[0] instanceof QQNode\AbstractBase) {
            throw new Caller('Pass either a an AbstractBase node or an array of QQNodes only');
        }

        $objNode = $objNodes[0]; // First node is the data node, the rest are for sorting.

        if (!$objNode->_ParentNode) {
            throw new Caller('First QQNode\AbstractBase cannot be a Top Level Node');
        }
        if (($objNode instanceof QQNode\ReverseReference) && !$objNode->isUnique()) {
            throw new Caller('Content QQNode\AbstractBase cannot go through any "To Many" association nodes.');
        }
