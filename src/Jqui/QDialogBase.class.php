 ...
 * @property-write string $DialogState Set whether this dialog is in an error or highlight (info) state. Choose on of QDialog::STATE_NONE, QDialogState::STATE_ERROR, QDialogState::stateHighlight(write-only)
 *
...
class DialogBase extends DialogGen
{
    ...
    public function __construct($objParentObject = null, $strControlId = null)
    {
        ...
        if ($this->blnAutoRemove) {
            ...
            $this->addAction(new QDialog_CloseEvent(10), new \QCubed\Action\AjaxControl($this, 'dialog_Close'));
        }
    }
    ...
    protected function stylingJs()
    {
        ...
        if ($this->strDialogState) {
            ...
            switch ($this->strDialogState) {
                case QDialog::STATE_ERROR:
                    $strIcon = 'alert';
                    break;

                case QDialog::STATE_HIGHLIGHT:
                    $strIcon = 'info';
                    break;
            }
    ...
    public function setButtonStyle($strButtonId, $styles)
    {
        QApplication::executeJavaScript(
            sprintf('$j("#%s").next().find("button[data-btnid=\'%s\']").css(%s)', $this->getJqControlId(), $strButtonId,
                \QCubed\Js\Helper::toJsObject($styles))
        );
    }
