    ...
    static public function php_type($jsType)
    {
        ...
        switch ($jsType) {
            ...
            case 'array[]':
                return 'array[]';
            default:
                return 'mixed';    // was JsClosure, which was limiting. Js\Closure, or Js\VarName could work here.
        }
    }
...
{
    public $oldEventClassName;
    public $eventClassName;
    public $eventName;
    public $arrArgs;

    public function __construct($strOldClass, $strQcClass, $name, $origName, $jsType, $description, $phpType = null)
    {
        ...
        if (stripos($jsType, 'function') === 0) {
            ...
            $this->eventName = $strQcClass . $name;
        } else {
            $this->eventName = $jsType;
        }
        $this->eventClassName = $strQcClass . $name;
        $this->oldEventClassName = $strOldClass . '_' . $name;
    }
}
...
class JqDoc
{
    ...
    public function __construct($strOldClass, $strJqClass, $strJqSetupFunc, $strQcClass, $strQcBaseClass)
    {
        $this->strOldClass = $strOldClass;
        ...
        if ($strQcClass === null) {
            if ($this->strJqClass !== null) {
                $this->strQcClass = $this->strJqClass;
            }
        } else {
...
class JqControlGen extends \QCubed\Codegen\AbstractBase
{
    ...
    public function GenerateControl($objJqDoc, $strOutDirControls, $strOutDirControlsBase)
    {
        foreach ($objJqDoc->events as $event) {
            $mixArgumentArray = array('event' => $event);
            $strResult = $this->EvaluatePHP(__DIR__ . '/jq_event.tpl.php', $mixArgumentArray);
            $strOutFileName = $strOutDirControlsBase . '/Event/' . $event->eventClassName . '.php';
        ...
        $strOutFileName = $strOutDirControlsBase . '/' . $objJqDoc->strQcClass . 'Base.php';
