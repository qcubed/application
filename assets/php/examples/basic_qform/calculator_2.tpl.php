...
<div id="instructions">
	...
	<p><em>What</em> the validation checks for is dependent on the control you are using.  In general,
		...
		<strong>\QCubed\Control\IntegerTextBox</strong> here to have Qforms ensure that the data entered in our two textboxes are
		valid integers.</p>

	<p>So we will utilize the \QCubed\Project\Control\FormBase's validation in our application by doing the following:</p>
	<ul>
		<li>Set our <strong>btnCalculate</strong> button's <strong>CausesValidation</strong> property to true</li>
		<li>Use <strong>\QCubed\Control\IntegerTextBox</strong> classes</li>
	...
	<p>For more advanced users, note that <strong>CausesValidation</strong> can also be set to <strong>\QCubed\Control\ControlBase::CAUSES_VALIDATION_SIBLINGS_AND_CHILDREN</strong>
		or <strong>\QCubed\Control\ControlBase::CAUSES_VALIDATION_SIBLINGS_ONLY</strong>.  This functionality is geared for developers who are creating more
