...
<div id="instructions">
	...
	of this here is the <strong>\QCubed\Event\KeyPress</strong> we added below.  As the user enters in data into the textbox,
	we make an AJAX call to update the label.  However, in order to make the system a bit more usable
	and streamlined, we have added a half-second (500 ms) delay on the <strong>\QCubed\Event\KeyPress</strong>, so that 
	...
	<p>Note that we maybe could have used a \QCubed\Event\Change on the textbox to achieve a similar effect.  But
	realize that \QCubed\Event\Change (which utilizes a javascript <strong>onchange</strong> event handler) will only be
