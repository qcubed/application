...
<div id="instructions">
	...
	<p>Note that what we are doing is combining multiple actions together into an action array (e.g. <b>\QCubed\Action\ActionBase[]</b>).
	Also note that this action array is defined on two different controls: the button (as a <b>\QCubed\Event\Click</b>)
	and the textbox (as a <b>\QCubed\Event\EnterKey</b>).</p>
	
	<p>Also note that we also add a <b>\QCubed\Action\Terminate</b> action to the textbox in response to
	the <b>\QCubed\Event\EnterKey</b>.  The reason for this is that on some browsers, hitting the enter
	key in a textbox would cause the form to do a traditional form.submit() call.  Given the way
	Qforms operates with named actions, and especially given the fact that this \QCubed\Project\Control\FormBase is using AJAX-based
	actions, we do <i>not</i> want the browser to be haphazardly performing submits.</p>
	
	<p>Finally, while this example uses <b>\QCubed\Action\Ajax</b> to make that an AJAX-based call to the PHP
	<b>AddListItem</b> method, note that this example can just as easily have made the call to
	<b>AddListItem</b> via a standard <b>\QCubed\Action\Server</b>.  The concept of combining multiple actions
